# ======================
# 🔑 USTAWIENIA API BYBIT
# ======================
API_KEY = "kMtSFs9PewJ6YsZiNQ"
API_SECRET = "twAFN52iq00Mr3A1ORdui0H2yp9EBkLwj1OA"

# ======================
# ⚙️ PARAMETRY BOTA
# ======================
SYMBOL = "WIFUSDT"  # Domyślny symbol
ALLOWED_SYMBOLS = [
    "WIFUSDT", "COAIUSDT", "ZECUSDT", "ZKUSDT", "TNSRUSDT", "ALLOUSDT", "OMUSDT",
    "NEARUSDT", "TRUMPUSDT", "STRKUSDT", "POPCATUSDT", "TRUSTUSDT", "MMTUSDT", "APRUSDT", "UAIUSDT",
]
TESTNET = False

# ======================
# 💬 POWIADOMIENIA
# ======================
DISCORD_WEBHOOK_URL = "https://discordapp.com/api/webhooks/1392636936723763210/nf-ZLx2Tz_nlen9eDwUeeTiiLDSlVR6yRvNGILNFLLpNsOJiXJxyO5EHD5DGhqQ4U2SZ"

# ======================
# 💰 TRYB HANDLU (TYLKO PROCENT)
# ======================
POSITION_VALUE = 0.50   # 1.0 = 100% dostępnego kapitału
